...
if (user_name && user_mail && user_password) { let query = "INSERT INTO user(user_name, user_mail, user_password) " ;
...
user.get("/", async (req, res, next) => {
    ...
    const rows = await db.query(query);
    return res.status(200).json({ code: 200, message: "Usuarios obtenidos exitosamente", data: rows });
}); 
